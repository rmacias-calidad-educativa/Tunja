# app.py — Versión solo para Tunja (con filtro de grados)
# Streamlit 1.37.x
...
# --- Config de página ---
st.set_page_config(
    page_title="Reporte Institucional — Innova Schools Tunja",
...
TARGET_SEDE_CANON = "Tunja"
TARGET_SEDE_KEYS = {"tunja"}
...
if TARGET_SEDE_DATASET is None:
    st.error("❌ No se encontró la sede 'Tunja' en el archivo.")
    st.stop()

# --- Filtrar todo a Tunja ---
dl0 = dl0[dl0["Sede"] == TARGET_SEDE_DATASET].copy()
df1 = df1[df1["Sede"] == TARGET_SEDE_DATASET].copy()

# --- Listas de áreas y grados disponibles (en Tunja) ---
...
with st.sidebar:
    ...
    if grados_disp:
        ...
        grado_opt = "Todos"
        st.caption("No hay grados 3/5/7/9 disponibles en Tunja.")
